...
class ANPInterface:
    ...
    def __init__(
        self, tool_name: str, interface_data: dict[str, Any], anp_client: ANPClient
    ):
        """
    ...
    async def execute(self, arguments: dict[str, Any]) -> dict[str, Any]:
        ...
        try:
                ...
                if isinstance(value, str):
                    ...
                    if (value.startswith("{") and value.endswith("}")) or (
                        ...
                        try:
                            ...
                            logger.info(
                                f"Parsed JSON parameter {key}: {value} -> {parsed_value}"
                            )
                        except json.JSONDecodeError:
                            processed_arguments[key] = value
                            logger.warning(
                                f"Failed to parse JSON parameter {key}: {value}"
                            )
                    else:
                        processed_arguments[key] = value
                else:
            ...
            logger.info(
                f"🔵 [ANP INTERFACE] Processed arguments: {processed_arguments}"
            )
            ...
            logger.info(
                f"🔵 [ANP INTERFACE] Executing tool call: {self.tool_name} -> {self.method_name} at {server_url}"
            )
            logger.info(
                f"🔵 [ANP INTERFACE] JSON-RPC request payload: {json.dumps(rpc_request, ensure_ascii=False, indent=2)}"
            )
            ...
            try:
                ...
                logger.info(
                    f"🟢 [ANP INTERFACE] HTTP response status: {response.get('status_code')}"
                )
...
class ANPInterfaceConverter:
    ...
    def convert_to_openai_tools(
        ...
        if interface_type == "jsonrpc_method":
            try:
                ...
                if result:
                    logger.debug(
                        f"Successfully converted {interface_type} to OpenAI Tools format"
                    )
                return result
            except Exception as e:
            ...
            logger.warning(
                f"Unsupported interface type: {interface_type}. Only JSON-RPC and OpenRPC methods are supported."
            )
            return None
    ...
    def _convert_jsonrpc_method(self, interface_data: dict[str, Any]) -> dict[str, Any]:
        ...
        description = interface_data.get(
            "description", f"JSON-RPC method: {method_name}"
        )
    ...
    def _convert_openrpc_method(self, interface_data: dict[str, Any]) -> dict[str, Any]:
        ...
        if not description:
            description = interface_data.get(
                "summary", f"OpenRPC method: {method_name}"
            )
    ...
    def _convert_jsonrpc_params_to_schema(
    ...
    def _convert_openrpc_params_to_schema(
        self, params: list[dict[str, Any]], components: dict[str, Any] = None
    ) -> dict[str, Any]:
        ...
        for key, value in schema.items():
            if key == "properties" and isinstance(value, dict):
                ...
                for prop_name, prop_schema in value.items():
                    resolved_properties[prop_name] = self._resolve_schema_refs(
                ...
                for item in value:
                    if isinstance(item, dict):
                        resolved_list.append(
                            self._resolve_schema_refs(item, components)
                        )
                    else:
