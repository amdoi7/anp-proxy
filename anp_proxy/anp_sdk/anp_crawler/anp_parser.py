...
class ANPDocumentParser:
    ...
    def parse_document(
        self, content: str, content_type: str, source_url: str
    ) -> dict[str, Any]:
        """
    ...
    def _extract_interfaces_from_agent_description(self, data: dict) -> list[dict]:
        ...
        if "interfaces" in data:
            for interface_def in data["interfaces"]:
                ...
                if (
                    interface_def.get("type") == "StructuredInterface"
                    and interface_def.get("protocol") == "openrpc"
                    and "content" in interface_def
                ):
                    # Extract OpenRPC interfaces from embedded content
                    content = interface_def["content"]
                    if isinstance(content, dict) and self._is_openrpc_interface(
                        content
                    ):
                        ...
                        logger.warning(
                            f"Invalid OpenRPC content in StructuredInterface: {interface_def.get('description', 'unknown')}"
                        )
                else:
    ...
    def _is_jsonrpc_interface(self, data: dict) -> bool:
        ...
        return isinstance(data, dict) and (
            "jsonrpc" in data
            or "methods" in data
            or ("id" in data and "method" in data)
        )
