...
async def demo_embedded_openrpc():
    ...
    for interface in parsed_result["interfaces"]:
        method_name = interface.get(
            "method_name", interface.get("description", "Unknown")
        )
        print(f"\n  接口: {method_name}")
        ...
        if "components" in interface:
            print(
                f"    组件定义: {len(interface.get('components', {}).get('schemas', {}))}"
            )
                ...
                if tool["function"]["name"] == "createBooking":
                    ...
                    if "guestInfo" in bd_props:
                        ...
                        if "properties" in guest_info:
                            print(
                                f"    ✅ $ref 解析成功: guestInfo 包含 {len(guest_info['properties'])} 个属性"
                            )
                            print(
                                f"       属性: {list(guest_info['properties'].keys())}"
                            )
    ...
    with patch(
        ...
        crawler = ANPCrawler(
            did_document_path="test/did.json", private_key_path="test/private_key.json"
        )
