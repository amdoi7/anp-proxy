...
class TestANPCrawler(unittest.IsolatedAsyncioTestCase):
    ...
    def setUp(self):
        ...
        with open(self.test_data_dir / "test_data_agent_description.json") as f:
            self.agent_description_data = json.load(f)

        # Load OpenRPC test data
        with open(self.test_data_dir / "test_data_openrpc.json") as f:
            self.openrpc_data = json.load(f)

        # Load embedded OpenRPC test data
        with open(self.test_data_dir / "test_data_embedded_openrpc.json") as f:
        ...
        self.test_embedded_openrpc_url = (
    ...
    async def test_fetch_text_embedded_openrpc(self, mock_auth_header):
        ...
        self.assertEqual(
            content_json["content"], json.dumps(self.embedded_openrpc_data)
        )
        ...
        self.assertIsNotNone(
            booking_interface, "createBooking interface should be found"
        )
        ...
        if "bookingDetails" in properties:
            ...
            if "guestInfo" in bd_properties:
                ...
                self.assertIn(
                    "properties", guest_info, "$ref for guestInfo should be resolved"
                )
                self.assertIn("firstName", guest_info["properties"])
                self.assertIn("email", guest_info["properties"])
    ...
    async def test_caching_functionality(self, mock_auth_header):
        ...
        await crawler.fetch_text(self.test_agent_url)
        self.assertEqual(
            crawler._client.fetch_url.call_count, 1
...
class TestANPDocumentParser(unittest.TestCase):
    ...
    def setUp(self):
        ...
        with open(test_data_dir / "test_data_agent_description.json") as f:
            self.agent_description_data = json.load(f)
        with open(test_data_dir / "test_data_openrpc.json") as f:
            self.openrpc_data = json.load(f)
        with open(test_data_dir / "test_data_embedded_openrpc.json") as f:
            self.embedded_openrpc_data = json.load(f)
    ...
    def test_parse_invalid_json(self):
        ...
        result = self.parser.parse_document(
            invalid_content, "application/json", "test_url"
        )
...
class TestANPInterface(unittest.TestCase):
    ...
    def test_unsupported_interface_type(self):
        ...
        unsupported_interface = {"type": "unsupported_type", "method_name": "test"}
