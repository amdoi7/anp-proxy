...
class ANPXEncoder:
    ...
    def encode_http_request(
        ...
        try:
            ...
            base_size = (
                5 + len(request_id.encode('utf-8')) +  # REQUEST_ID TLV
                5 + meta_size +                        # HTTP_META TLV
                5                                      # HTTP_BODY TLV header
            )

            if len(body) == 0 or base_size + len(body) <= self.chunk_size:
                # Single message
                return [self._create_single_request_message(request_id, http_meta, body)]
            else:
                # Chunked messages
                return self._create_chunked_request_messages(request_id, http_meta, body)
    ...
    def encode_http_response(
        ...
        try:
            ...
            base_size = (
                5 + len(request_id.encode('utf-8')) +  # REQUEST_ID TLV
                5 + meta_size +                        # RESP_META TLV
                5                                      # HTTP_BODY TLV header
            )

            if len(body) == 0 or base_size + len(body) <= self.chunk_size:
                # Single message
                return [self._create_single_response_message(request_id, resp_meta, body)]
            else:
                # Chunked messages
                return self._create_chunked_response_messages(request_id, resp_meta, body)
    ...
    def _create_chunked_request_messages(
        ...
        first_chunk_overhead = (
            5 + len(request_id.encode('utf-8')) +      # REQUEST_ID TLV
            5 + len(meta_json.encode('utf-8')) +       # HTTP_META TLV
            5 + 5 + 5 + 5                              # CHUNK_IDX + CHUNK_TOT + FINAL_CHUNK + HTTP_BODY TLVs
        )

        first_chunk_body_size = max(0, self.chunk_size - first_chunk_overhead)
        remaining_chunk_size = self.chunk_size - (5 + len(request_id.encode('utf-8')) + 5 + 5 + 5 + 5)  # Overhead for subsequent chunks
        ...
        if len(body) <= first_chunk_body_size:
            ...
            additional_chunks = (remaining_body + remaining_chunk_size - 1) // remaining_chunk_size
        ...
        for chunk_idx in range(total_chunks):
            ...
            if chunk_idx == total_chunks - 1:
                message.add_tlv_field(TLVTag.FINAL_CHUNK, 1)
    ...
    def _create_chunked_response_messages(
        ...
        last_chunk_overhead = (
            5 + len(request_id.encode('utf-8')) +      # REQUEST_ID TLV
            5 + len(meta_json.encode('utf-8')) +       # RESP_META TLV
            5 + 5 + 5 + 5                              # CHUNK_IDX + CHUNK_TOT + FINAL_CHUNK + HTTP_BODY TLVs
        )

        regular_chunk_size = self.chunk_size - (5 + len(request_id.encode('utf-8')) + 5 + 5 + 5)  # Overhead for regular chunks
        ...
        if len(body) <= last_chunk_body_size:
            ...
            regular_chunks = (body_for_regular_chunks + regular_chunk_size - 1) // regular_chunk_size
